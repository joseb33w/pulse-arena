...
//
// SUPABASE_ANON_KEY is the project's *publishable* key — safe to ship in the
// browser; row access is governed by the Realtime channel, not this string.
...
// Lightweight, non-secret diagnostics for headless verification (two tabs in
// the same room should each see rx climb as the other broadcasts).
window.__pulseNet = { id: _userId, room: "", connected: false, rx: 0, tx: 0 };
...
window.gameNet = {
  ...
  connectRoom(room) {
    ...
    _channel.on("broadcast", { event: "msg" }, (e) => {
      window.__pulseNet.rx++;
      _emit(e.payload);
    });
    _channel.subscribe((status) => {
      if (status === "SUBSCRIBED") {
        window.__pulseNet.connected = true;
        _emit({ t: "_connected", room: room, you: _userId });
      } else if (status === "CHANNEL_ERROR" || status === "TIMED_OUT") {
        window.__pulseNet.connected = false;
        _emit({ t: "_disconnected" });
      }
    });
  },
